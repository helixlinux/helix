    ...
    std::optional<Config> Config::load(const std::string &path)
    {
        try
        {
            ...
            if (yaml["socket"])
            {
                ...
                if (socket["path"])
                    config.socket_path = socket["path"].as<std::string>();
                if (socket["backlog"])
                    config.socket_backlog = socket["backlog"].as<int>();
                if (socket["timeout_ms"])
                    config.socket_timeout_ms = socket["timeout_ms"].as<int>();
            }
            ...
            if (yaml["rate_limit"])
            {
                ...
                if (rate["max_requests_per_sec"])
                    config.max_requests_per_sec = rate["max_requests_per_sec"].as<int>();
            }
