...
namespace helixd
{
    ...
    void Handlers::register_all(IPCServer &server)
    {
        ...
        // Security handlers
        server.register_handler(Methods::ALERTS, [](const Request &req)
    ...
    Response Handlers::handle_alerts_get(const Request & /*req*/)
    {
        ...
        if (has_alert)
        {
            alerts.push_back({{"id", "security-updates-missing"},
                              {"severity", "warning"},
        ...
        return Response::ok({{"package_manager", package_manager},
    ...
    Response Handlers::handle_security_patches_install(const Request & /*req*/)
    {
        ...
        return Response::ok({{"installed", true},
