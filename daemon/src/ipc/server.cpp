...
namespace helixd
{
    ...
    RateLimiter::RateLimiter(int max_per_second)
        : max_per_second_(max_per_second), window_start_(std::chrono::steady_clock::now())
    {
    }
    ...
    IPCServer::IPCServer(const std::string &socket_path, int max_requests_per_sec)
        : socket_path_(socket_path), rate_limiter_(max_requests_per_sec)
    {
    }
    ...
    bool IPCServer::setup_permissions()
    {
        ...
        // The socket directory (/run/helix/) provides additional access control if needed.
