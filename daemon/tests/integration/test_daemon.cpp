 ...
 */
class MockService : public helixd::Service {
public:
...
protected:
    void SetUp() override {
        ...
        temp_dir_ = fs::temp_directory_path() / ("helixd_daemon_test_" + std::to_string(getpid()));
...
TEST_F(DaemonTest, InstanceReturnsSameDaemon) {
    auto& daemon1 = helixd::Daemon::instance();
    auto& daemon2 = helixd::Daemon::instance();
...
TEST_F(DaemonTest, GetServiceReturnsNullptrForUnregistered) {
    ...
    auto* service = daemon.get_service<helixd::IPCServer>();
