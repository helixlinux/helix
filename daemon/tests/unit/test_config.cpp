...
protected:
    void SetUp() override {
        ...
        temp_dir_ = fs::temp_directory_path() / ("helixd_test_" + std::to_string(getpid()) + "_" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));
...
TEST_F(ConfigTest, DefaultsReturnsValidConfig) {
    auto config = helixd::Config::defaults();
    
    EXPECT_EQ(config.socket_path, "/run/helix/helix.sock");
...
TEST_F(ConfigTest, LoadReturnsNulloptForNonexistentFile) {
    auto result = helixd::Config::load("/nonexistent/path/config.yaml");
...
log_level: 2
    ...
    auto result = helixd::Config::load((temp_dir_ / "valid.yaml").string());
...
socket:
  path: /tmp/partial.sock
    ...
    auto result = helixd::Config::load((temp_dir_ / "partial.yaml").string());
...
socket:
  ...
  backlog: "not a number"
    ...
    auto result = helixd::Config::load((temp_dir_ / "invalid.yaml").string());
    EXPECT_FALSE(result.has_value());
}
...
log_level: 1
    ...
    auto result = helixd::Config::load((temp_dir_ / "invalid_values.yaml").string());
    EXPECT_FALSE(result.has_value());
}
...
TEST_F(ConfigTest, SaveCreatesValidYamlFile) {
    ...
    // Reload and verify
    auto reloaded = helixd::Config::load(save_path);
...
TEST_F(ConfigTest, ExpandPathFunctionExpandsTilde) {
    std::string path = "~/.helix/test";
    std::string expanded = helixd::expand_path(path);
    
    EXPECT_NE(expanded[0], '~');
    EXPECT_TRUE(expanded.find("/.helix/test") != std::string::npos);
}
...
TEST_F(ConfigTest, ConfigManagerReturnsSameInstance) {
    auto& instance1 = helixd::ConfigManager::instance();
    auto& instance2 = helixd::ConfigManager::instance();
    
    EXPECT_EQ(&instance1, &instance2);
}

TEST_F(ConfigTest, ConfigManagerLoadReturnsDefaultsOnFailure) {
    auto& manager = helixd::ConfigManager::instance();
