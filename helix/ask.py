"""Natural language query interface for Helix.
...
class LearningTracker:
    ...
    def progress_file(self) -> Path:
        ...
        if self._progress_file is None:
            try:
                self._progress_file = Path.home() / ".helix" / "learning_history.json"
            except RuntimeError:
                # Fallback for restricted environments where home is inaccessible
                import tempfile

                self._progress_file = (
                    Path(tempfile.gettempdir()) / ".helix" / "learning_history.json"
                )
    ...
    def record_topic(self, question: str) -> None:
        ...
        without file locking. If multiple helix ask processes run concurrently,
...
- Point two here"""
    ...
    def _call_fake(self, question: str, system_prompt: str) -> str:
        ...
        fake_response = os.environ.get("HELIX_FAKE_RESPONSE", "")
        if fake_response:
