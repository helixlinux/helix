...
1. HELIX_PROVIDER=ollama environment variable (for explicit Ollama mode)
2. API key environment variables: ANTHROPIC_API_KEY, OPENAI_API_KEY
3. Encrypted storage (~/.helix/environments/helix.json) - secure Fernet-encrypted storage
4. Cached key location (~/.helix/.api_key_cache)
5. Saved Ollama provider preference in ~/.helix/.env (HELIX_PROVIDER=ollama)
6. API keys in ~/.helix/.env
...
from helix.branding import console, cx_print
...
HELIX_DIR = ".helix"
HELIX_ENV_FILE = ".env"
HELIX_CACHE_FILE = ".api_key_cache"
...
class APIKeyDetector:
    ...
    def __init__(self, cache_dir: Path | None = None):
        ...
        Args:
            cache_dir: Directory for caching key location info.
                      Defaults to ~/.helix
        """
        self.cache_dir = cache_dir or (Path.home() / HELIX_DIR)
        self.cache_file = self.cache_dir / HELIX_CACHE_FILE
    ...
    def detect(self) -> tuple[bool, str | None, str | None, str | None]:
        ...
        Detection order (highest priority first):
        1. HELIX_PROVIDER=ollama environment variable
        2. API key environment variables (ANTHROPIC_API_KEY, OPENAI_API_KEY)
        3. Encrypted storage (~/.helix/environments/helix.json)
        ...
        """
        # Check for explicit HELIX_PROVIDER=ollama in environment variable first
        if os.environ.get("HELIX_PROVIDER", "").lower() == "ollama":
    ...
    def _check_encrypted_storage(self) -> tuple[bool, str, str, str] | None:
        """Encrypted storage is not available in the stripped build."""
        return None
    ...
    def _ask_to_save_ollama_preference(self) -> None:
        ...
        print(
            f"\nSave Ollama as default provider to ~/{HELIX_DIR}/{HELIX_ENV_FILE}? [Y/n] ", end=""
        )
        try:
        ...
        if response != "n":
            ...
            cx_print(f"✓ Provider preference saved to ~/{HELIX_DIR}/{HELIX_ENV_FILE}", "success")

    def _save_provider_to_env(self, provider: str) -> None:
        """Save provider preference to ~/.helix/.env with file locking."""
        try:
            env_file = Path.home() / HELIX_DIR / HELIX_ENV_FILE
            self._locked_read_modify_write(
                env_file, self._update_or_append_key, "HELIX_PROVIDER", provider
            )
        except Exception as e:
            cx_print(f"Warning: Could not save provider to ~/.helix/.env: {e}", "warning")
    ...
    def _ask_to_save_key(self, key: str, provider: str) -> None:
        ...
        print(f"\nSave to ~/{HELIX_DIR}/{HELIX_ENV_FILE}? [Y/n] ", end="")
        try:
            response = input().strip().lower()
        except (EOFError, KeyboardInterrupt):
            response = "n"
        ...
        if response != "n":
            ...
            target_env = Path.home() / HELIX_DIR / HELIX_ENV_FILE
            self._cache_key_location(key, provider, str(target_env))
            cx_print(f"✓ Key saved to ~/{HELIX_DIR}/{HELIX_ENV_FILE}", "success")

    def _maybe_save_found_key(self, key: str, provider: str, source: str) -> None:
        """Offer to save a detected key to ~/.helix/.env."""
        ...
        if response != "n":
            ...
            self._cache_key_location(key, provider, str(target_env))
            cx_print("✓ Key saved to ~/.helix/.env", "success")
    ...
    def _get_check_locations(self) -> list[tuple]:
        ...
        return [
            ...
            (Path.home() / HELIX_DIR / HELIX_ENV_FILE, ["ANTHROPIC_API_KEY", "OPENAI_API_KEY"]),
    ...
    def _save_key_to_env(self, key: str, provider: str):
        ...
        try:
            ...
            cx_print(f"Warning: Could not save key to ~/.helix/.env: {e}", "warning")
...
def setup_api_key() -> tuple[bool, str | None, str | None]:
    ...
    """
    detector = APIKeyDetector()
    silent = os.environ.get("HELIX_SILENT_OUTPUT", "0") == "1"

    # Try auto-detection first
    found, key, provider, source = detector.detect()
    if found:
        # Only show "Found" message for non-default locations
        # ~/.helix/.env is our canonical location, so no need to announce it
