...
if TYPE_CHECKING:
    from helix.semantic_cache import SemanticCache
...
class CommandInterpreter:
    ...
    def __init__(
        ...
        self._hardware_context = self._gather_hardware_context()
    ...
    def _get_system_prompt(self, simplified: bool = False) -> str:
        ...
        """
        hw = getattr(self, "_hardware_context", "")

        if simplified:
            base = """You must respond with ONLY a JSON object. No explanations, no markdown, no code blocks.
...
- Return ONLY the JSON object"""
        ...
        base = """You are a Linux system command expert. Convert natural language requests into safe, validated bash commands.
...
{{\"commands\": [\"command1\", \"command2\"]}}"""
    ...
    def _call_fake(self, user_input: str) -> list[str]:
        ...
        fake_commands_env = os.environ.get("HELIX_FAKE_COMMANDS")
        if not fake_commands_env:
            raise RuntimeError("HELIX_FAKE_COMMANDS environment variable not set")

        try:
            data = json.loads(fake_commands_env)
            commands = data.get("commands", [])
            if not isinstance(commands, list):
                raise ValueError("Commands must be a list in HELIX_FAKE_COMMANDS")
            return [cmd for cmd in commands if cmd and isinstance(cmd, str)]
        except json.JSONDecodeError as e:
            raise RuntimeError(f"Failed to parse HELIX_FAKE_COMMANDS: {str(e)}")
    ...
    def parse(self, user_input: str, validate: bool = True) -> list[str]:
        ...
        cache_system_prompt = (
            self._get_system_prompt() + f"\n\n[helix-cache-validate={bool(validate)}]"
        )
