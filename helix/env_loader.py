"""
Environment Loader for Helix Linux
...
2. User's Helix config directory (~/.helix/.env)
3. System-wide config (/etc/helix/.env)

Later files do NOT override earlier ones - the first value found wins.

Issue: https://github.com/helixlinux/helix/issues/XXX
"""
...
def get_env_file_locations() -> list[Path]:
    ...
    # 2. User's home directory .helix folder
    home_helix_env = Path.home() / ".helix" / ".env"
    locations.append(home_helix_env)

    # 3. System-wide config (Linux only)
    if os.name == "posix":
        system_env = Path("/etc/helix/.env")
