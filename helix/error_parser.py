...
class ErrorParser:
    ...
    COMMAND_TEMPLATES: dict[str, dict[str, str]] = {
        ...
        "yum": {
            ...
            "fix": "sudo yum check-update && sudo yum-complete-transaction",
        ...
        "pacman": {
            ...
            "autoremove": "sudo pacman -Rns --noconfirm $(pacman -Qtdq)",
            "remove": "sudo pacman -R --noconfirm {package}",
        },
        "zypper": {
            "update": "sudo zypper refresh",
            "install": "sudo zypper --non-interactive install {package}",
            "fix": "sudo zypper --non-interactive verify",
            "clean": "sudo zypper clean --all",
            "autoremove": "sudo zypper --non-interactive packages --unneeded",
            "remove": "sudo zypper --non-interactive remove {package}",
        },
        "apk": {
    ...
    ERROR_PATTERNS = [
        ...
        {
            "pattern": r"nothing provides\s+([^\s]+)\s+needed by\s+([^\s]+)",
            "category": ErrorCategory.DEPENDENCY_MISSING,
            "confidence": 0.92,
            "fixes": [
                "Install missing dependency: sudo apt-get install {group_0}",
                "Refresh repositories and retry installation",
                "Enable required repository for {group_1}",
            ],
            "auto_fix": "sudo apt-get install -y {group_0}",
        },
        {
            "pattern": r"error:\s+failed to prepare transaction \(could not satisfy dependencies\)",
            "category": ErrorCategory.DEPENDENCY_MISSING,
            "confidence": 0.88,
            "fixes": [
                "Synchronize package database and retry",
                "Install missing dependency explicitly",
                "Try full system upgrade before install",
            ],
            "auto_fix": None,
        },
        {
            "pattern": r"Problem:\s+package\s+([^\s]+)\s+requires\s+([^\s]+)",
            "category": ErrorCategory.DEPENDENCY_MISSING,
            "confidence": 0.9,
            "fixes": [
                "Install dependency: sudo apt-get install {group_1}",
                "Try distro sync / full upgrade",
            ],
            "auto_fix": "sudo apt-get install -y {group_1}",
        },
        # Package not found
        {
        ...
        {
            "pattern": r"conflicting requests|conflicts with installed package",
            "category": ErrorCategory.CONFLICT,
            "confidence": 0.8,
            "fixes": [
                "Remove the conflicting package, then retry installation",
                "Use package manager's conflict resolution mode",
                "Pin a compatible package version",
            ],
            "auto_fix": None,
        },
        # Broken packages
    ...
    def __init__(self):
        ...
        self.cmd = self.COMMAND_TEMPLATES.get(self.package_manager, self.COMMAND_TEMPLATES["apt"])
    ...
    def _translate_apt_commands(self, text: str) -> str:
        """Translate apt-centric fix text/commands to active package manager."""
        translated = text
        translated = translated.replace("sudo apt-get update", self.cmd["update"])
        translated = translated.replace("sudo apt-get install -f -y", self.cmd["fix"])
        translated = translated.replace("sudo apt-get install -f", self.cmd["fix"])
        translated = translated.replace("sudo apt-get clean", self.cmd["clean"])
        translated = translated.replace("sudo apt-get autoremove -y", self.cmd["autoremove"])
        translated = translated.replace("sudo apt-get install -y", self.cmd["install"].replace(" {package}", ""))
        translated = translated.replace("sudo apt-get install", self.cmd["install"].replace(" {package}", ""))
        translated = translated.replace("apt-cache search", "dnf search" if self.package_manager == "dnf" else "yum search" if self.package_manager == "yum" else "pacman -Ss" if self.package_manager == "pacman" else "zypper search" if self.package_manager == "zypper" else "apk search" if self.package_manager == "apk" else "apt-cache search")
        return translated
    ...
    def _generate_fixes(self, matches: list[ErrorMatch], error_message: str) -> list[str]:
            ...
            for pattern_def in self.compiled_patterns:
                if (
                    ...
                    for fix_template in pattern_def["fixes"]:
                        ...
                        fix = self._translate_apt_commands(fix)
        ...
        if not fixes:
            fixes = [
                f"Update package lists: {self.cmd['update']}",
                f"Fix broken dependencies: {self.cmd['fix']}",
            ...
            for pattern_def in self.compiled_patterns:
                if (
                    ...
                    if auto_fix:
                        ...
                        if "{" in auto_fix and "}" in auto_fix:
                            continue
                        auto_fix = self._translate_apt_commands(auto_fix)
