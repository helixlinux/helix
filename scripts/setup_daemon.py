...
AUDIT_DB_PATH = Path.home() / ".helix" / "history.db"
...
def init_audit_db() -> bool:
    ...
    Creates ~/.helix directory if needed and sets up a SQLite database
...
CONFIG_FILE = "/etc/helix/daemon.yaml"
CONFIG_EXAMPLE = DAEMON_DIR / "config" / "helixd.yaml.example"
HELIX_ENV_FILE = Path.home() / ".helix" / ".env"
...
def check_daemon_built() -> bool:
    """
    Check if the helixd daemon binary has been built.

    Checks for the existence of the helixd binary at DAEMON_DIR / "build" / "helixd".

    Returns:
        bool: True if the daemon binary exists, False otherwise.
    """
    return (DAEMON_DIR / "build" / "helixd").exists()
...
def build_daemon(with_tests: bool = False) -> bool:
    """
    Build the helixd daemon from source.
...
def install_daemon() -> bool:
    """
    Install the helixd daemon system-wide.
...
def main() -> int:
    """
    Interactive setup wizard for the Helix daemon.

    Guides the user through building and installing the helixd daemon.
    ...
    console.print(
        "[bold cyan]║           Helix Daemon Interactive Setup                    ║[/bold cyan]"
    )
    console.print(
        "[bold cyan]╚══════════════════════════════════════════════════════════════╝[/bold cyan]\n"
    )
    ...
    init_audit_db()
    log_audit_event("setup_started", "Helix daemon interactive setup started")
    ...
    console.print("[cyan]  systemctl status helixd       # Check daemon status[/cyan]")
    console.print("[cyan]  journalctl -u helixd -f       # View daemon logs[/cyan]")
    console.print("[cyan]  helix daemon ping             # Test daemon connection[/cyan]")
    console.print("[cyan]  helix daemon version          # Get daemon version[/cyan]")

    if check_tests_built():
        console.print("\n[cyan]Test commands:[/cyan]")
        console.print("[cyan]  helix daemon run-tests        # Run all tests[/cyan]")
        console.print("[cyan]  helix daemon run-tests --unit # Run unit tests only[/cyan]")
        console.print(
            "[cyan]  helix daemon run-tests --integration  # Run integration tests[/cyan]"
        )
        console.print("[cyan]  helix daemon run-tests -t config      # Run specific test[/cyan]")
