#!/usr/bin/env python3
"""
Ollama Setup Script for Helix Linux
...
5. Test the model
6. Configure Helix to use Ollama
...
def get_available_models() -> list[dict[str, Any]]:
    """Get list of recommended models for Helix."""
    return [
        {
            "name": "llama3.2",
            "size": "2GB",
...
def prompt_model_selection(models: list[dict[str, Any]], installed: list[str]) -> str | None:
    ...
    print("\nAvailable Ollama models for Helix:\n")
...
def configure_helix(model_name: str) -> bool:
    """Configure Helix to use Ollama with the selected model."""
    print_info("Configuring Helix to use Ollama...")

    helix_dir = Path.home() / ".helix"
    helix_dir.mkdir(mode=0o700, exist_ok=True)

    config_file = helix_dir / "config.json"
    ...
    try:
        ...
        print_success("Helix configuration updated!")
...
def main():
    """Main setup flow."""
    parser = argparse.ArgumentParser(description="Set up Ollama for Helix Linux")
    parser.add_argument(
        "--model",
    ...
    print_header("Ollama Setup for Helix Linux")
    ...
    print(f"  • Test Helix: {Colors.OKGREEN}helix install nginx --dry-run{Colors.ENDC}")
    print(f"  • Chat with AI: {Colors.OKGREEN}helix ask 'how do I update my system?'{Colors.ENDC}")
